...
# Почему я не добавил courier_id в существующие таблицы

Я решил не добавлять courier_id в таблицу заказов, а создать отдельную таблицу‑связку dds.fct_deliveries, потому на мой взгляд это в большей степени  соответствует принципам нормализации.  
Так можно разделить разные бизнес‑сущности: заказы с ресторанами отдельно, а заказы с более широким контекстом отдельно.  Это позволяет хранить все метрики доставки (оценку, чаевые, время и т.п.) там, где легко добавлять новые поля , не нарушая структуру dimension, и при необходимости поддерживать историю или перевыдачу заказов.  
Я нахожу такой подход гибче и чище, при этом получаю все необходимые для витрины данные.
